...
#ifdef FLASH_KDA_TCGEN05_K2
    // Two reusable tcgen05 operand tiles.  K is only 16 bf16 values, but the
    // 128-byte swizzle gives every logical row a 128-byte physical stride.
    alignas(1024) uint8_t tcgen_a[128 * 128];
    alignas(1024) uint8_t tcgen_b[128 * 128];

    // Logical [CHUNK, D] buffers in the same layout as v/out.  temp0 is reused
    // for k@s and (v-k@s)*beta; temp1 holds U=INV@(...).
    alignas(128) cute::ArrayEngine<BF16, cute::cosize_v<MMALayout>> tcgen_temp0;
    alignas(128) cute::ArrayEngine<BF16, cute::cosize_v<MMALayout>> tcgen_temp1;

    alignas(8) uint64_t tcgen_mbarrier;
    uint32_t tcgen_tmem_addr[1];
#endif
};
...
__global__ void __launch_bounds__(NumThreads) _flash_kda_fwd_recurrence(
    ...
    // --- shared memory
#ifdef FLASH_KDA_TCGEN05_K2
    extern __shared__ __align__(1024) unsigned char shared_mem[];
#else
    ...
    if (warp_role == WarpRole::MMA) {
...
#ifdef FLASH_KDA_TCGEN05_K2
        // One TMEM allocation is reused for all matrix products and chunks of
        // this persistent recurrence CTA.  alloc/dealloc are warp-collective.
        const int compute_warp_id = compute_tid / 32;
        const int compute_lane_id = compute_tid % 32;
        const uint32_t tcgen_mbar_addr = static_cast<uint32_t>(
            __cvta_generic_to_shared(&shared_storage.tcgen_mbarrier));
        if (compute_warp_id == 0) {
            if (compute_lane_id == 0) {
                asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;" :
                             : "r"(tcgen_mbar_addr), "r"(1));
                asm volatile("fence.mbarrier_init.release.cluster;");
            }
            const uint32_t dst = static_cast<uint32_t>(
                __cvta_generic_to_shared(shared_storage.tcgen_tmem_addr));
            asm volatile(
        ...
        const uint32_t tcgen_tmem_addr = shared_storage.tcgen_tmem_addr[0];
        uint32_t tcgen_phase = 0;
#endif
        ...
        for (int t = 0; t < t_tiles; ++t) {
...
#ifdef FLASH_KDA_TCGEN05_K2
            // Experimental SM100 path.  It preserves the same chunk-level
            // equations and bf16 materialization points as the SM80 path, but
            // expresses every K2 GEMM as physical m128n128k16 tcgen05 tiles.
            // Logical M=16 products zero-pad rows 16..127.
            Tensor k_restored_t = make_tensor(
                make_smem_ptr(shared_storage.input[load_stage].k_restored.begin()),
                TransposedMMALayout{});
            Tensor tcgen_temp0 = make_tensor(
                make_smem_ptr(shared_storage.tcgen_temp0.begin()), MMALayout{});
            Tensor tcgen_temp1 = make_tensor(
                make_smem_ptr(shared_storage.tcgen_temp1.begin()), MMALayout{});

            const int tcgen_row = compute_warp_id * 32 + compute_lane_id;

            auto tcgen_run = [&] (bool accumulate) {
                // Generic shared stores become visible to the async tcgen proxy.
                asm volatile("fence.proxy.async.shared::cta;");
                compute_barrier.arrive_and_wait();
                if (compute_warp_id == 0 && flash_kda_tcgen05_elect_one()) {
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    const uint32_t a_base = static_cast<uint32_t>(
                        __cvta_generic_to_shared(shared_storage.tcgen_a));
                    const uint32_t b_base = static_cast<uint32_t>(
                        __cvta_generic_to_shared(shared_storage.tcgen_b));
                    const uint64_t a_desc = flash_kda_tcgen05_make_desc(
                        a_base, 0, 1024, 2);
                    const uint64_t b_desc = flash_kda_tcgen05_make_desc(
                        b_base, 0, 1024, 2);
                    flash_kda_tcgen05_issue(
                        tcgen_tmem_addr, a_desc, b_desc,
                        tcgen_mbar_addr, accumulate);
                }
                flash_kda_tcgen05_wait(tcgen_mbar_addr, tcgen_phase & 1u);
                asm volatile("tcgen05.fence::after_thread_sync;");
                compute_barrier.arrive_and_wait();
                ++tcgen_phase;
            };

            // Phase 1a: temp0 = k_decayed[16,128] @ state^T[128,128].
            // Eight K=16 tcgen05 operations accumulate in TMEM.
            #pragma unroll 1
            for (int kb = 0; kb < D / 16; ++kb) {
                for (int i = compute_tid; i < 128 * 16; i += kComputeThreads) {
                    const int row = i / 16;
                    const int kk = i % 16;
                    const BF16 a = row < CHUNK
                        ? k_decayed(row, kb * 16 + kk) : BF16(0);
                    *reinterpret_cast<BF16*>(&shared_storage.tcgen_a[
                        flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) = a;
                    *reinterpret_cast<BF16*>(&shared_storage.tcgen_b[
                        flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) =
                        s_acc(row, kb * 16 + kk);
                }
                tcgen_run(kb != 0);
            }
            for (int col = 0; col < D; col += 8) {
                const uint32_t src = tcgen_tmem_addr +
                    (static_cast<uint32_t>(compute_warp_id * 32) << 16) + col;
                float result[8];
                flash_kda_tcgen05_load_x8(src, result);
                if (tcgen_row < CHUNK) {
                    #pragma unroll
                    for (int i = 0; i < 8; ++i)
                        tcgen_temp0(tcgen_row, col + i) = BF16(result[i]);
                }
            }
            compute_barrier.arrive_and_wait();

            // Phase 1b: out = q_decayed @ state^T.
            #pragma unroll 1
            for (int kb = 0; kb < D / 16; ++kb) {
                for (int i = compute_tid; i < 128 * 16; i += kComputeThreads) {
                    const int row = i / 16;
            ...
            for (int col = 0; col < D; col += 8) {
                ...
                if (tcgen_row < CHUNK) {
                    #pragma unroll
                    for (int i = 0; i < 8; ++i)
                        out_tile(tcgen_row, col + i) = BF16(result[i]);
                }
            }
            compute_barrier.arrive_and_wait();

            // Elementwise residual/beta step at the same bf16 boundary as the
            // original Phase 3: temp0 = (v - k@s) * sigmoid(beta).
            for (int i = compute_tid; i < CHUNK * D; i += kComputeThreads) {
                const int row = i / D;
                const int col = i % D;
                const BF16 beta_value = BF16(sigmoid_tanh_approx_f32(
                    float(beta_tile(beta_smem_offset + row))));
                tcgen_temp0(row, col) =
                    (v_tile(row, col) - tcgen_temp0(row, col)) * beta_value;
            }
            compute_barrier.arrive_and_wait();

            // Phase 3: temp1(U) = INV[16,16] @ temp0[16,128].
            for (int i = compute_tid; i < 128 * 16; i += kComputeThreads) {
                const int row = i / 16;
                const int kk = i % 16;
                const BF16 a = row < CHUNK ? INV(row, kk) : BF16(0);
                *reinterpret_cast<BF16*>(&shared_storage.tcgen_a[
                    flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) = a;
                *reinterpret_cast<BF16*>(&shared_storage.tcgen_b[
                    flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) =
            ...
            for (int col = 0; col < D; col += 8) {
                ...
                if (tcgen_row < CHUNK) {
                    #pragma unroll
                    for (int i = 0; i < 8; ++i)
                        tcgen_temp1(tcgen_row, col + i) = BF16(result[i]);
                }
            }
            compute_barrier.arrive_and_wait();

            // Phase 4/5: out += Mqk[16,16] @ U[16,128].
            for (int i = compute_tid; i < 128 * 16; i += kComputeThreads) {
                const int row = i / 16;
                const int kk = i % 16;
                const BF16 a = row < CHUNK ? Mqk(row, kk) : BF16(0);
                *reinterpret_cast<BF16*>(&shared_storage.tcgen_a[
                    flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) = a;
                *reinterpret_cast<BF16*>(&shared_storage.tcgen_b[
                    flash_kda_tcgen05_swizzle_128b(row, kk * 2)]) =
            ...
            for (int col = 0; col < D; col += 8) {
                ...
                if (tcgen_row < CHUNK) {
                    #pragma unroll
                    for (int i = 0; i < 8; ++i) {
                        const BF16 product = BF16(result[i]);
                        out_tile(tcgen_row, col + i) =
                            out_tile(tcgen_row, col + i) + product;
                    }
                }
            }
            compute_barrier.arrive_and_wait();

            // Phase 6: state = state*g_total + k_restored^T @ U.
            for (int i = compute_tid; i < 128 * 16; i += kComputeThreads) {
                const int row = i / 16;
                const int kk = i % 16;
                *reinterpret_cast<BF16*>(&shared_storage.tcgen_a[
            ...
            for (int col = 0; col < D; col += 8) {
                ...
                float result[8];
                flash_kda_tcgen05_load_x8(src, result);
                const float decay = g_total(tcgen_row);
                #pragma unroll
                for (int i = 0; i < 8; ++i) {
                    s_acc_T(tcgen_row, col + i) = BF16(
                        bf16_to_f32(s_acc_T(tcgen_row, col + i)) * decay +
                        result[i]);
                }
            }
#else
