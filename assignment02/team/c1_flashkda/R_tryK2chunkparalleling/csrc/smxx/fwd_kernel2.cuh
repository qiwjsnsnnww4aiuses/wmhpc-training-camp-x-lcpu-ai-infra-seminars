...
__global__ void __launch_bounds__(NumThreads) _flash_kda_fwd_recurrence(
    ...
    // R carry-lookahead path intentionally represents aligned segments as
    // varlen sequences.  On that path blockIdx.x is therefore segment_idx,
    // changing K2 from grid=(sequence,head) to
    // grid=(sequence*segments,head) without changing the proven chunk body.
