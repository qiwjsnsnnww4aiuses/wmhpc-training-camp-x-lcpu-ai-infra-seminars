...
\newcommand{\good}[1]{\textcolor{KdaGreen}{\textbf{#1}}}
\newcommand{\bad}[1]{\textcolor{KdaRed}{\textbf{#1}}}
\newcommand{\key}[1]{\textcolor{KdaBlue}{\textbf{#1}}}
...
\begin{frame}{KDA：8192 个 token 为什么不能全部打散？}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.56\textwidth}
      ...
      \begin{tikzpicture}[
        node distance=6mm,
        box/.style={draw=KdaBlue,rounded corners,minimum width=19mm,minimum height=10mm,fill=KdaLight},
        state/.style={draw=KdaOrange,rounded corners,minimum width=12mm,minimum height=8mm,fill=KdaOrange!12},
        ...
        \node[align=center,below=11mm of s2,text=KdaGreen] (parallel)
          {可并行：不同 sequence、不同 head、chunk 内计算};
        \node[align=center,below=5mm of parallel,text=KdaRed] (serial)
          {不可直接并行：同一 sequence/head 的相邻状态更新};
      \end{tikzpicture}
...
\begin{frame}{从 Python 到 GPU：项目结构与执行链}
  ...
  \begin{tikzpicture}[
    node distance=4.5mm,
    box/.style={draw=KdaBlue,rounded corners,minimum width=37mm,minimum height=7mm,fill=KdaLight,align=center,font=\small},
    gpu/.style={draw=KdaOrange,rounded corners,minimum width=44mm,minimum height=8mm,fill=KdaOrange!12,align=center,font=\small},
    arr/.style={-{Stealth[length=2mm]},thick,KdaNavy}
  ]
    ...
    \node[gpu,below left=6mm and 3mm of launch] (k1) {K1: \code{fwd\_kernel1.cuh}\\chunk-parallel prepare};
    \node[gpu,below right=6mm and 3mm of launch] (k2) {K2: \code{fwd\_kernel2.cuh}\\state recurrence};
...
\begin{frame}{讨论 3：状态有依赖，并行度还能从哪里来？}
  \small
  \begin{tabular}{>{\bfseries}p{0.20\textwidth}p{0.33\textwidth}p{0.39\textwidth}}
    \toprule
    候选方案 & 潜在收益 & 反例 / 风险 \\
    \midrule
    多 head 合一个 CTA
