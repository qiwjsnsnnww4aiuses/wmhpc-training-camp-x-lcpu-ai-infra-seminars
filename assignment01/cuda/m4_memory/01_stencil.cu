...
int main() {
    ...
    stencil_dynamic<<<blocks, BLOCK, (BLOCK+RADIUS*2) * sizeof(float)>>>(d_in, d_out, n);
    CUDA_CHECK_KERNEL();
    CUDA_CHECK(cudaMemcpy(h_out, d_out, bytes, cudaMemcpyDeviceToHost));
    if (!check_close(h_out, h_ref, n)) REPORT(0);
    printf("dynamic PASS\n");
